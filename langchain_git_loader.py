...
class GitLoader(BaseLoader):
    ...
    Currently, supports only text files. Each document represents one file in the repository.
    The `path` points to the local Git repository, and the `branch` specifies the branch to load files from. By default, it loads from the `main` branch.
    """
    ...
    def lazy_load(self) -> Iterator[Document]:
        ...
        if self.relative_path:
            # Load files from specified relative paths
            for relative_path in self.relative_path:
                full_path = os.path.join(self.repo_path, relative_path)
                if not os.path.exists(full_path):
                    raise ValueError(
                        f"The relative path '{relative_path}' does not exist "
                        f"in the repository at '{self.repo_path}'."
                    )
                yield from self._load_files(full_path)
        else:
            # Load all files in the repository
            yield from self._load_files(self.repo_path)

    def _load_files(self, path: str) -> Iterator[Document]:
